...
export interface CommissionReportData {
  ...
  calculations: { provider: string; branch: string; productsCount: number; productIncentive: number; serviceCommission: number; laserIncentive: number; fixedIncentive: number; manualAdjustment: number; bonusExtra: number; grossTotal: number; cleaningContribution: number; netTotal: number; status: string; accountNumber?: string | null }[]

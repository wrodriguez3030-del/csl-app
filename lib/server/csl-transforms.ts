...
export function fromDb(entity: string, row: Row): Row {
  switch (entity) {
    ...
    case "reportes":
      return { ID: row.report_id, Fecha: row.fecha, EquipoID: row.equipo_id, Sucursal: row.sucursal, Empresa: row.empresa, Cliente: row.cliente, Domicilio: row.domicilio, Ciudad: row.ciudad, Modelo: row.modelo, Serie: row.serie, Numero: row.numero, Tipo: row.tipo, EstadoEquipo: row.estado_equipo, Prioridad: row.prioridad, Problema: row.problema, Correccion: row.correccion, Observaciones: row.observaciones, Checklist: row.checklist, P_Cabeza: row.p_cabeza, P_Totales: row.p_totales, Atendio: row.atendio, PowerSourceNumber: row.power_source_number, PowerSourceSerial: row.power_source_serial, FiberSerial: row.fiber_serial, HV: row.hv_value, Joules: row.joules_value, BS: row.bs_value, BC: row.bc_value, HVRef: row.hv_ref_value, VDC: row.vdc_value, Voltage: row.voltage_value, TX: row.tx_value, Software: row.software_version, PiezasJSON: row.piezas_json, PartesTexto: row.partes_texto, FirmaCliente: row.firma_cliente, FirmaTecnico: row.firma_tecnico, Fotos: row.fotos }
    case "inventario":

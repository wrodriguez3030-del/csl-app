...
export function BiAlertasPage() {
  ...
  const STATUS_OPTS = [
    { v: "abierta", l: `Abiertas (${counts.abierta || 0})` },
    { v: "todas", l: `Todas (${counts.total || 0})` },
    { v: "revisada", l: `Revisadas (${counts.revisada || 0})` },
              ...
              <li key={a.id} className="flex flex-wrap items-start justify-between gap-2 rounded-xl border border-[color:var(--brand-border)] p-3">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {a.period_month ? <span className="text-[11px] text-muted-foreground">· {MESES_ALERT[a.period_month]} {a.period_year}</span> : null}
                    <span className={`rounded-full px-2 py-0.5 text-[10px] ${a.status === "abierta" ? "bg-rose-50 text-rose-600" : a.status === "descartada" ? "bg-slate-100 text-slate-500" : "bg-emerald-50 text-emerald-600"}`}>{a.status}</span>
                  </div>

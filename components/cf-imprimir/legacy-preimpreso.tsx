...
/**
 * CF PARA IMPRIMIR · Modo "Pre-impreso (físico)" — LEGACY conservado.
 *
 * Imprime SOLO los campos + QR sobre un certificado FÍSICO ya pre-impreso,
 * con calibración de alineación por impresora. Es el flujo histórico del
 * módulo; se conserva íntegro para no romper la operación existente. El flujo
 * moderno (generación digital de 3 diseños) vive en gift-cert-editor.
 */
...
export function LegacyPreimpresoPage() {
  ...
  const createPrintPdf = async () => {
    ...
    const shortDate = form.fecha ? form.fecha.split("-").reverse().join("/") : ""
    const drawLine = (label: string, value: string, x: number, y: number) => {
      const safeLabel = label.startsWith("CORTES") ? "CORTESIA DE" : label
      page.drawText(`${safeLabel}: ${normalizeCertificateText(value)}`, {
    ...
    drawLine("OTORGADO A", form.otorgadoA, 160, 188)
    drawLine("CORTESÍA DE", form.cortesiaDe, 160, 153)
    drawLine("SERVICIO", form.validoPor, 44, 70)
    drawLine("FECHA", shortDate, 44, 52)
    drawLine("SUCURSAL", form.sucursal || "", 44, 34)
    ...
    if (validationUrl) {
      try {
        ...
        page.drawImage(qr, { x: certificateArea.x + certificateArea.width - 78 + calibration.qrOffsetX, y: certificateArea.y + 28 + calibration.qrOffsetY, width: 46, height: 46 })
        page.drawText(form.codigo, { x: certificateArea.x + certificateArea.width - 84 + calibration.qrOffsetX, y: certificateArea.y + 20 + calibration.qrOffsetY, size: 4.5, font, color: printBlack })
      } catch {}
    }
  ...
  const createCalibrationPdf = async () => {
    ...
    drawMark("OTORGADO A", 160, 188)
    drawMark("CORTESIA DE", 160, 153)
    drawMark("SERVICIO", 44, 70)
    drawMark("FECHA", 44, 52)
    drawMark("SUCURSAL", 44, 34)
    drawMark("QR", certificateArea.width - 55, 51)
  ...
  return (
    <div className="space-y-6">
      <style>{`
        ...
        .cf-code{position:absolute;right:0.35in;bottom:0.22in;font:700 0.08in Arial;color:#111;text-align:center}
        .cf-qr{display:block;width:0.72in;height:0.72in;margin:0 auto 0.03in}
        @media print{
      ...
      <div className="no-print flex items-center justify-between gap-3">
        <div>
          <h3 className="text-base font-bold">Pre-impreso (físico)</h3>
          <p className="text-sm text-muted-foreground">Coloca el certificado físico en la impresora. Se imprimirán solo los campos y el QR.</p>
        </div>
        ...
        <Card className="no-print border-primary/40">
          ...
          <CardContent className="grid gap-4 md:grid-cols-3 lg:grid-cols-6">
            ...
            <div className="md:col-span-3 lg:col-span-6 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              ...
              <span>Si sale muy a la derecha usa X negativo. Si sale muy abajo usa Y positivo. Imprime en tamaño real / 100%.</span>
            </div>
          </CardContent>
        </Card>
      ) : null}

...
import { GIFT_ACTIONS, handleGiftAction } from "./handlers/gift-certificates"
import {
  hrSha256,
  HR_DAILY_BASE,
  hrAudit,
...
async function dispatchAction(action: string, params: ActionParams, user: ActionUser) {
  ...
  // Módulos con archivo propio. `_handlers.ts` había llegado a 5.805 líneas y
  // 361 `case`: cada módulo que sale de aquí se lee y se prueba por separado.
  if (GIFT_ACTIONS.has(action)) return handleGiftAction(action, params, user)
  ...
  switch (action) {
    ...
    case "commitCommissionImport":
      return await commission.commitCommissionImport(params, user)
    case "updateCommissionCalculation":
    ...
    case "appendReservationsRows":
      return await commission.appendReservationsRows(params, user)
    case "finalizeReservationsImport":
      return await commission.finalizeReservationsImport(params, user)
    case "voidCommissionImport":
      return await commission.voidCommissionImport(params, user)
